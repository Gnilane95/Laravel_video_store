    ...
    <form action="{{ route('videos.update', $video->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mx-56 shadow-lg p-5">
            ...
            <div class="">
                ...
                <input type="text" name="title" class="block  w-72 rounded-lg border-gray-400 mt-2" value="{{ old('title',$video->title) }}">
                <x-error-msg name="title" />
            </div>
            {{-- Actors --}}
            <div class="mt-5">
                <label for="actors" class="font-bold">Acteurs du film</label>
                <input type="text" name="actors" class="block  w-72 rounded-lg border-gray-400 mt-2" value="{{ old('actors',$video->actors) }}">
                <x-error-msg name="actors" />
            </div>
            {{-- Nationality --}}
            <div class="mt-5">
                <label for="nationality" class="font-bold">Nationalité</label>
                <input type="text" name="nationality" class="block w-72 rounded-lg border-gray-400 mt-2" value="{{ old('nationality',$video->nationality) }}">
                <x-error-msg name="nationality" />
            </div>
            {{-- Year --}}
            <div class="mt-5">
                <label for="year_created" class="font-bold">Année de création</label>
                <input type="text" name="year_created" class="block w-72 rounded-lg border-gray-400 mt-2" value="{{ old('year_created',$video->year_created) }}">
